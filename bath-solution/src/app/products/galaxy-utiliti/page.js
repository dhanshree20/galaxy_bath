...
export default function GalaxyUtiliti() {
    ...
    >
      ...
      >
        ...
        >
          ...
          <p><strong style={{ color: "#d4af37" }}>Material:</strong> {product.material}</p>
          <p style={{ marginBottom: "20px" }}>
            <strong style={{ color: "#d4af37" }}>Color:</strong> {product.color}
          </p>

          <h2 style={{ color: "#d4af37" }}>Sizes Available</h2>
          {product.sizes.map((s, i) => (
            <div key={i} style={{ margin: "8px 0" }}>• {s}</div>
          ))}

          <h2 style={{ color: "#d4af37", marginTop: "25px" }}>Basic Combo</h2>
          {product.basicCombo.map((c, i) => (
            <div key={i}>• {c}</div>
          ))}

          <h2 style={{ color: "#d4af37", marginTop: "20px" }}>Optional Features</h2>
          {product.optionalFeatures.map((c, i) => (
            <div key={i}>• {c}</div>
          ))}

          <h2 style={{ color: "#d4af37", marginTop: "20px" }}>Faucet Sets</h2>
          {product.faucetSets.map((c, i) => (
            <div key={i}>• {c}</div>
          ))}
        </div>
      </div>
