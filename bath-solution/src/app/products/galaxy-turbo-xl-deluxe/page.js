...
export default function GalaxyTurboXLDeluxe() {
    ...
    <section style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ color: "#d4af37", fontSize: "20px", marginBottom: "10px" }}>Sizes Available</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: "10px", marginBottom: "18px" }}>
            {product.sizes.map((s,i) => (<div key={i} style={{ background: "rgba(255,255,255,0.06)", padding: "10px 14px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.08)" }}>{s}</div>))}
          </div>

          <h3 style={{ color: "#d4af37", marginTop: "6px" }}>Basic Combo</h3>
          <ul style={{ marginTop: "8px" }}>{product.basicCombo.map((it, idx) => <li key={idx} style={{ marginBottom: "6px", opacity: 0.95 }}>• {it}</li>)}</ul>

          <h3 style={{ color: "#d4af37", marginTop: "18px" }}>Optional Features</h3>
          <ul style={{ marginTop: "8px" }}>{product.optionalFeatures.map((it, idx) => <li key={idx} style={{ marginBottom: "6px", opacity: 0.95 }}>• {it}</li>)}</ul>

          <h3 style={{ color: "#d4af37", marginTop: "18px" }}>Faucet Sets</h3>
          <ul style={{ marginTop: "8px" }}>{product.faucetSets.map((it, idx) => <li key={idx} style={{ marginBottom: "6px", opacity: 0.95 }}>• {it}</li>)}</ul>
        </div>
      </div>
    </section>
  );
